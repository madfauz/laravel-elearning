<x-app-layout>
    ...
    <div class="card shadow-sm border rounded-3">
        <div class="w-[90%] mx-auto">
            <form action="{{ route('manage-user.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 my-6">
                ...
                <div class="flex justify-between gap-4 w-[100%] md:w-auto">
                    <select name="role" class="border pl-3 pr-12 py-2 rounded w-[50%] md:w-auto">
                    <option value="">Semua Role</option>
                    <option value="teacher" {{ request('role') == 'teacher' ? 'selected' : '' }}>Teacher</option>
                    <option value="student" {{ request('role') == 'student' ? 'selected' : '' }}>Student</option>
                </select>

                <button type="submit" class="bg-green-400 text-white px-4 py-2 rounded w-[50%] md:w-auto">Filter</button>
                </div>
            </form>
        </div>
                            ...
                            <td class="flex justify-center gap-2">
                                @if (Route::has('manage-user.update'))
                                    <a class="w-1/3 py-2 px-0"
                                        href="{{ route('manage-user.update', $user->user_id) }}">
                                ...
                                @if (Route::has('manage-user.destroy'))
                                    ...
                                    <x-confirm-modal id="delete_user_{{ $user->user_id }}"
                                        message="Are you sure you want to delete {{ $user->name }}?" okLabel="Delete"
                                        cancelLabel="Cancel" :url="route('manage-user.destroy', $user->user_id)" method="DELETE" />
                                @endif
                            </td>
                        </tr>
                    @empty
