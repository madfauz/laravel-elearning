<x-app-layout>
    <div class="w-[90%] md:w-1/2 mx-auto my-12">
        <a href="{{ route('manage-quiz.index', $quiz->course_id) }}"
            class="text-sm text-gray-600 flex items-center gap-2 mb-6 cursor-pointer">
            <x-bx-arrow-back class="w-4 h-4 text-gray-600" />
            {{ __('Back') }}
        </a>

        <form id="update_quiz_form" method="POST"
            action="{{ route('manage-quiz.update', ['course_id' => $quiz->course_id, 'quiz_id' => $quiz->quiz_id]) }}">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $quiz->title)"
            ...
            <div class="mt-4">
                ...
                <textarea id="description" name="description" rows="3" style="resize:none"
                    class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $quiz->description) }}</textarea>
            ...
            <div class="mt-6">
                ...
                <div id="questions_wrapper">
                    @foreach ($quiz->questions as $qIndex => $question)
                        <div class="question_item border p-4 rounded mb-4 bg-gray-50" data-q-index="{{ $qIndex }}"
                            data-next-opt-index="{{ count($question->options) }}">
                            <div class="flex items-start gap-3">
                                <div class="flex-1">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Question Text</label>
                                    <textarea name="questions[{{ $qIndex }}][question_text]" rows="2" style="resize:none"
                                        class="block w-full border-gray-300 rounded-md shadow-sm">{{ old("questions.$qIndex.question_text", $question->question_text) }}</textarea>
                                </div>
                                <button type="button"
                                    class="remove-question text-red-600 text-sm hover:underline mt-6">Remove</button>
                            </div>
                            ...
                            <div class="mt-3">
                                ...
                                <select name="questions[{{ $qIndex }}][type]"
                                    ...
                                    <option value="short_answer"
                                        {{ $question->type == 'short_answer' ? 'selected' : '' }}>Short Answer</option>
                                </select>
                            </div>

                            <div class="options-area mt-4 {{ $question->type == 'short_answer' ? 'hidden' : '' }}">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-medium">Options</h4>
                                    <button type="button"
                                        class="add-option px-2 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600">
                                        + Add Option
                                    </button>
                                </div>
                                ...
                                <div class="options-wrapper space-y-2">
                                    @foreach ($question->options as $oIndex => $option)
                                        <div class="option_item flex items-center gap-2"
                                            ...
                                            <button type="button"
                                                class="remove-option text-red-500 text-xs hover:underline">Remove</button>
                                        </div>
                                    @endforeach
                                </div>
                                <p class="hint text-xs text-gray-500 mt-1">
                                    For True/False, only one option can be correct. For Short Answer, options are
                                    hidden.
                                </p>
                            </div>
                        </div>
        ...
        <div class="flex items-center justify-end mt-6">
            ...
            <x-confirm-modal id="update_confirmation" message="Are you sure you want to update this quiz?"
                okLabel="Update" cancelLabel="Cancel" formId="update_quiz_form" />
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('add_question_btn').addEventListener('click', function() {
            ...
            <div class="flex items-start gap-3">
                ...
                <button type="button" class="remove-question text-red-600 text-sm hover:underline mt-6">Remove</button>
            </div>
            ...
            <div class="options-area mt-4">
                <div class="flex items-center justify-between mb-2">
                    <h4 class="font-medium">Options</h4>
                    <button type="button" class="add-option px-2 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600">+ Add Option</button>
                </div>
